...
uint8_t coolant_get_state()
{
  ...
  if (HAL_GPIO_ReadPin(COOLANT_FLOOD_PORT,COOLANT_FLOOD_BIT) == 0 )
#else
  if (HAL_GPIO_ReadPin(COOLANT_FLOOD_PORT,COOLANT_FLOOD_BIT) == 1 )
#endif
...
/*
    ...
    if (bit_isfalse(
		...
		,(1 << COOLANT_FLOOD_BIT))) {
  #else
    if (bit_istrue(HAL_GPIO_ReadPin(COOLANT_FLOOD_PORT,COOLANT_FLOOD_BIT)
		GPIO_ReadOutputData(COOLANT_FLOOD_PORT),(1 << COOLANT_FLOOD_BIT)))
    {
  #endif
    cl_state |= COOLANT_STATE_FLOOD;
  }
