...
#endif
#else
  if ( HAL_GPIO_ReadPin(SPINDLE_DIRECTION_PORT,SPINDLE_DIRECTION_BIT) )
...
//  TIM_CtrlPWMOutputs(TIM1, DISABLE);
  HAL_TIM_PWM_Stop(SPINDLE_TIMER_H , SPINDLE_TIMER_CHANNEL);
...
void spindle_set_speed(SPINDLE_PWM_TYPE pwm_value)
{
  ...
  {
    TIM_CtrlPWMOutputs(SPINDLE_TIMER, ENABLE);
#ifdef INVERT_SPINDLE_ENABLE_PIN
    ResetSpindleEnablebit();
#else
    SetSpindleEnablebit();
  ...
  if (pwm_value == SPINDLE_PWM_OFF_VALUE)
  {
    TIM_CtrlPWMOutputs(SPINDLE_TIMER, DISABLE);
  }
  else
  {
    TIM_CtrlPWMOutputs(SPINDLE_TIMER, ENABLE);
