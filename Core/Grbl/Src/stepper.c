...
void st_wake_up()
{
  ...
  { 
	  HAL_GPIO_WritePin(STEPPERS_DISABLE_BIT_GPIO_Port,STEPPERS_DISABLE_BIT_Pin,1);
	  //SetStepperDisableBit();
  }
  else 
  { 
	  HAL_GPIO_WritePin(STEPPERS_DISABLE_BIT_GPIO_Port,STEPPERS_DISABLE_BIT_Pin,0);
  ...
  TIM6->ARR = st.step_pulse_time - 1;
  /*
  TIM6->EGR = TIM_AUTORELOAD_PRELOAD_ENABLE
...
void grbl_tim6_IRQHandler(void)
{
